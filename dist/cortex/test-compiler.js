...
async function testCompiler() {
    ...
    const compiler = new compiler_1.Compiler(apiKey);
    ...
    for (const testCase of testCases) {
        ...
        try {
            const unit = await compiler.compile(testCase.input, userId);
            ...
            if (unit.when.type === 'event') {
                console.log(`   Source: ${unit.when.source}`);
                console.log(`   Event: ${unit.when.event}`);
                if (unit.when.filter) {
                    console.log(`   Filter: ${unit.when.filter}`);
                }
            }
            else if (unit.when.type === 'schedule') {
                console.log(`   Cron: ${unit.when.cron}`);
            }
            if (unit.if.length > 0) {
                console.log(`\n📋 Conditions (${unit.if.length}):`);
                unit.if.forEach((cond, i) => {
                    if (cond.type === 'eval') {

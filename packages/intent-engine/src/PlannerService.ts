...
import { ProviderAwareToolFilter } from './ProviderAwareToolFilter';
import { EventEmitter } from 'events';
import { StreamChunk } from './stream/types';
import { MessageType } from './conversation/types';
import { ILLMClient, IToolProvider, IToolFilter, ChatMessage, ChatOptions, ToolConfig } from '@aso/interfaces';
...
export class PlannerService extends EventEmitter {
  ...
  private createLegacyLLMAdapter(groq: Groq): ILLMClient {
    ...
    return {
      ...
      chatStream: async function* (options: ChatOptions) {
...
public async generatePlan(
  ...
  let availableTools: Array<Pick<ToolConfig, 'name' | 'description' | 'category' | 'parameters'>>;
  if (this.toolFilter && userId) {
    ...
    if (filteredTools.length === 0) {
      ...
      availableTools = filteredTools.map((tool: ToolConfig) => ({
        name: tool.name,
    ...
    availableTools = this.toolProvider.getAllTools().map((tool: ToolConfig) => ({
      name: tool.name,
      description: tool.description,
      category: tool.category,
      parameters: tool.parameters
